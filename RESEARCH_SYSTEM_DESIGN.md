...
初稿：2026-09-06；本次完善：2026-09-08。版本：v0.2。状态：冻结设计基线；逐项实施与
验收状态以 `RESEARCH_IMPLEMENTATION_PROGRESS.md` 为准。
...
| Covered Call | 用已有整手现股换取权利金 | 现股覆盖、上涨封顶、提前指派、除息日与滚动 |
| Collar | 用空头 Call 补贴保护性 Put | 两腿净权利金、保护区间、上涨封顶和分腿风险 |
...
Covered Call 不是下跌保险：权利金只能缓冲有限损失，标的大跌时仍承受主要下行风险，
上涨超过执行价后收益被限制。系统只允许每张空头 Call 由匹配标的和交割物的足额现股覆盖，
卖出现股前必须重新检查覆盖。Collar 在此基础上买入 Put，保护范围和上涨封顶都必须按净
权利金、两腿价差及可能的提前指派共同报告。
...
阶段 2a 首先只做标准 QQQ/SPY 合约的固定保护性 Put。候选规则、目标期限、到期前退出窗口及保险预算在验证阶段冻结；没有合格合约可以保持未保护状态并披露缺口。`coverage` 分别报告名义覆盖、Delta 覆盖和压力情景损失，避免用一个百分比暗示完全保险。独立 Long Put、Bear Put Spread、现金担保卖 Put、Covered Call 和 Collar 放入阶段 2b；其中带空腿的策略必须先通过指派、现股覆盖与现金占用验收。
...
当前已实现只读 `GET /api/v2/accounts/current` 和
`GET /api/v2/signals/latest?strategy_id=...&horizon=...`，两者只消费 Python 端重新验证、
内容寻址后原子发布的文件，不提供账户写入或执行入口。缺当前有效信号返回明确的不可用
状态，不回退到历史名单；不支持的预测周期返回可用周期列表，不缩放已有模型结果冒充
新预测。`GET /api/v2/runs/{run_id}`、账户历史版本和 `GET /api/v2/plans/{plan_id}` 仍是后续
读接口，不应在路由落地前写成已完成能力。
