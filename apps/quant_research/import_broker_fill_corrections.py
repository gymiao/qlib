"""Import read-only broker fill reversals into local simulator state."""
...
from .broker_fill_corrections import import_broker_fill_correction_batch, load_broker_fill_correction_batch
...
def main(argv: list[str] | None = None) -> int:
    ...
    batch = load_broker_fill_correction_batch(
        args.corrections, package, as_of, timedelta(minutes=args.max_age_minutes),
    )
    result, _ = SimulatorStateStore(args.simulator_state).transact(
        lambda simulator: import_broker_fill_correction_batch(batch, package, simulator.ledger),
